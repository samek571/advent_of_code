class Solution:
    def numDistinctIslands(self, mat: List[List[int]]) -> int:
            ...
            for c in range(cols):
                ...
                if mat[r][c] == 1:
                    ...
                    for pos in positions:
                        topLeft[0] = min(topLeft[0], pos[0])
                        topLeft[1] = min(topLeft[1], pos[1])
                    
                    # Translate island to origin
                    for i in range(len(positions)):
                        positions[i][0] -= topLeft[0]
                        positions[i][1] -= topLeft[1]
                    
                    allDistinctIslands.add(str(sorted(positions)))
                    #allDistanceCountMap = {}
                    ...
                    # We are asked the number of different islands
